...
function populateFilterOptions(){const items=getItems();const subjects=[...new Set(items.map(item=>item.subject).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-CN'));const categories=[...new Set(items.map(item=>item.category).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-CN'));const knowledgeTags=[...new Set(items.flatMap(item=>item.knowledge_tags||[]).filter(Boolean))].sort((a,b)=>a.localeCompare(b,'zh-CN'));['q-filter-subj','pick-subject','rec-subject','inst-subject'].forEach(id=>setSelectOptions(id,subjects,'全部科目'));['q-filter-category','pick-category','rec-filter-category','inst-category'].forEach(id=>setSelectOptions(id,categories,'全部分类'));['q-filter-ktag','pick-ktag','rec-filter-ktag','inst-ktag'].forEach(id=>setSelectOptions(id,knowledgeTags,'全部知识点'));if(typeof renderLabelFilterOptions==='function')renderLabelFilterOptions()}
...
function uiToast(text,options={}){let node=document.getElementById('ui-toast');if(!node){node=document.createElement('div');node.id='ui-toast';node.className='ui-toast';document.body.appendChild(node)}node.className=`ui-toast ${options.kind||''}`.trim();node.innerHTML=`<span class="ui-toast-text">${escapeHtml(text)}</span>${(options.actions||[]).map((a,i)=>`<button type="button" data-ui-toast-act="${i}">${escapeHtml(a.label)}</button>`).join('')}`;node.onclick=event=>{const btn=event.target.closest('[data-ui-toast-act]');if(!btn)return;const action=(options.actions||[])[Number(btn.dataset.uiToastAct)];node.classList.remove('show');clearTimeout(UI_TOAST_TIMER);if(action&&typeof action.onClick==='function')action.onClick()};requestAnimationFrame(()=>node.classList.add('show'));clearTimeout(UI_TOAST_TIMER);UI_TOAST_TIMER=setTimeout(()=>node.classList.remove('show'),options.duration||(options.actions?.length?6000:2600))}
// uiDialog({title, body(html), okText, cancelText, danger, focus}) → Promise<{ok, values:{id:value}}>
