...
function fbRenderPanel(){
  ...
  panel.innerHTML=`${head}
    ...
    ${row.correct===false&&row.uid?'<button class="btn sm fb-board-add" data-fb-act="board">📋 加入展示板</button>':''}
...
function fbBindPanel(){
  ...
  panel.addEventListener('click',event=>{
    ...
    if(action==='board'){if(current?.row?.uid&&typeof boardQuickAdd==='function')boardQuickAdd(current.row.uid);return}
    if(!current)return;
