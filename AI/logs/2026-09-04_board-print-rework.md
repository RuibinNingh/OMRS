...
> **事实勘误（2026-09-06）**：以下记录描述当时的重做背景和设计过程；当前实现已采用 `printed.pages`、`printed.cursor`、`printed.items` 的浏览器实测纸面记录，`last_printed_page` 仅作为旧字段忽略。当前没有 `binding_marks` 或 3 孔/26 孔孔位圆圈，板级 `gap_lines` 默认 2 行；仅保留极浅 `.bind-line` 装订导引虚线。
