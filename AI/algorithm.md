...
推荐接口返回的顺序是复习工作台的基础顺序；前端均衡模式只在该顺序内按科目轮选，不重新计算一套优先级。列表条目的 `_source`、`due_date`、`mastery` 和 `_overdue_days` 来自后端投影，避免浏览器依据本地日期或静态字段推断推荐原因。用户标记筛选支持任一或全部匹配，难度/熟练度范围和到期状态筛选只裁剪候选，不改变后端排序。
