...
4. **组装**：`_build_export_data()` 产出 `{meta, questions, feedback, answers}`；`_build_html()` 读 `export_templates/{variant}.css` 与 `.js`，并把本地 `assets/vendor/katex/` 的 CSS/JS/字体一起内联。数据 JSON 会做 `</` 转义防提前闭合脚本，最终仍是单个自包含 HTML。
...
- 暂无导出端专门待办；后续如扩展 Markdown 语法，应优先保持 A4 与屏幕版模板一致。
