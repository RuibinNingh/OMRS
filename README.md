...
| **展示板** | 持久化题目引用集合，可拖拽/菜单排序、按标记同步、设置留白与装订孔位，导出左题右空 A4「错题集」；打印后记录纸面，新加的题可**只补印新增**并接在原纸空白处，页码始终是绝对页码 |
...
- **展示板左题右空版**：以 `boards.json` 的持久化题目引用为输入，左侧按题目顺序排版（KaTeX / 表格 / 长图切白缝 / 跨页续排）、右侧完全留白；标题固定为「错题集」，支持 22mm 装订边、题间留白、孔位标记、附答案页、绝对页码；「仅打印新增」把新题排在纸面记录的续排位置，已打印区域留白，把原纸放回打印机即可补印。
