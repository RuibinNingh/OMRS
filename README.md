...
展示板锁定后仍可添加新题并补印：增删引用、排序和调整未打印题留白不会清空旧纸面记录；改动已打印区域的版式才需要明确确认重印。
...
| **复习 Session** | 调度器挑题 → 反馈录入。复习调度页只保留推荐与导出，Session 题目详情和反馈提交集中在独立的「反馈录入」工作台；支持分批提交，序号始终对应 Session 原始题目顺序 |
...
| **展示板** | 持久化题目引用集合，可拖拽/菜单排序、按标记同步、设置右侧与题间留白，导出左题右空 A4「错题集」；题栏与右侧留白默认各占可分配宽度的 50%，页面左右各留 10mm，不额外预留装订区或绘制装订导引线；打印后记录纸面，新加的题可**只补印新增**并接在原纸空白处，页码始终是绝对页码 |
